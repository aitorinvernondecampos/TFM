...
# # # # # # # # # #                                                                               #
#                                                                                                 #
# Estudio comparativo de la potencia de la versión asintótica de PERMANOVA (MANTA) con            #
# respecto a MANOVA, profundizando en la afectación de la variación del nivel α de                #
...
# # # # # # # # # # #                                                                             #
#                                                                                                 #
# Estudio de la posible invarianza frente a la transformación de los datos del método             #
# asintótico PEMANOVA, implementado en MANTA, con respecto a su potencia estadística,             #                                     
# teniendo en cuenta diferentes situaciones de simulación del conjunto de datos.                  #
# Simulación de los datos bajo un algoritmo que implementa un 3-símplex (n=3 => tetraedro).       #
#                                                                                                 #
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 

# => Se usará una función de simulación de datos original "Sim.simplex", que depende de otras dos
#    funciones igualmente originales ( y ), que implementa el algoritmo 3-símplex deseado.
#    Esta función depende de las variables:
#         - ch: niveles del factor escogido
#         - q: número de respuestas .𝜏. q ∈ [3, 5]
#          <=> (q>5) no debería subir el tiempo de computación en exceso pero sí llevar a "sitios extraños"!!!
#         - n: tamaño de la muestra
#         - loc: indica la ubicación del modelo generador "simplex" (p / position / loc) .𝜏. f(q)
#               e.g.1 Si (loc = 1/3) ≡> Para un 3-símplex nos encontramos en el "Centro del simplex" ≡ (1/3, 1/3, 1/3)
#               e.g.2 Si (loc -> 1) ≡> Para un 3-símplex nos encontramos en un "Vértice del simplex" ≡ (1, 0, 0)
#         - delta: donde Δ = 0 evalua H0 y Δ > 0 evaluará H1
#         - hk: grado de heterocedasticidad, que determinará la constancia o no de la varianza de los errores,
#               determinando así la validez de una de las hipótesis básicas sobre las que se asienta el modelo
#               de regresión lineal.
#         - stdev: 
#         - pdist: 
# => Se simularán todas las combinaciones "Δ - q - Loc" que sean posibles, siempre y cuando no
#    no den errores en la ejecución del algoritmo.
# => Como para "mvnorm", la opción de variar "n" en vez de "Δ" no es computacionalmente tan efectiva, ya que eleva
#    demasiado los tiempos de cálculo.


# a) Analizar todas las combinaciones de valores posibles de las variables Δ - q - Loc dentro del símplex.
...
# .𝜏. unique(DF_Results_Graph_2graph$Datos)[c(1)] <=> Sin transformar
# .𝜏. unique(DF_Results_Graph_2graph$Datos)[c(1, 2, 4)] <=> Sin transformar + log-ratio + sqrt
# .𝜏. unique(DF_Results_Graph_2graph$Datos)[1:4] <=> Sin transformar + log-ratio + clr + sqrt
DF_Results_Graph_2graph <- DF_Results_Graph_2graph[DF_Results_Graph_2graph$Datos ==
                                                     c(unique(DF_Results_Graph_2graph$Datos)[c(1)]), ]
...
if (modelSim != ""){
  ...
  # Restringimos los valores a simular:
  Alpha_values_sim <- Alpha_values[2:3]
  S_values_sim <- S_values[2]
  n_values_sim <- n_values[2]
  Var_values_sim <- Var_values[]
  q_values_sim <- q_values[]
...
for (Datos_graph in unique(DF_Results_Graph_2graph$Datos)){
  for (S_graph in unique(DF_Results_Graph_2graph$S)){
    for (alpha_graph in unique(DF_Results_Graph_2graph$alpha)){
      ...
      # PDF
      graph_path = file.path(paste0(getwd(), "/Gráficas/PDF/∆-Potencia MANTA-MANOVA/", "alpha = ",
                                    sprintf("%1.3f", alpha_graph)))
      dir.create(graph_path, recursive = TRUE, showWarnings = FALSE)
      ...
      # PNG
      graph_path = file.path(paste0(getwd(), "/Gráficas/PNG/∆-Potencia MANTA-MANOVA/", "alpha = ",
                                    sprintf("%1.3f", alpha_graph)))
      dir.create(graph_path, recursive = TRUE, showWarnings = FALSE)
...
for (Metodo_graph in unique(DF_Results_Graph_2graph$Método)){
  for (S_graph in unique(DF_Results_Graph_2graph$S)){
    for (alpha_graph in unique(DF_Results_Graph_2graph$alpha)){
      ...
      # PDF
      graph_path = file.path(paste0(getwd(), "/Gráficas/PDF/Estudio invarianza/", "alpha = ",
                                    sprintf("%1.3f", alpha_graph)))
      dir.create(graph_path, recursive = TRUE, showWarnings = FALSE)
      ...
      # PNG
      graph_path = file.path(paste0(getwd(), "/Gráficas/PNG/Estudio invarianza/", "alpha = ",
                                    sprintf("%1.3f", alpha_graph)))
      dir.create(graph_path, recursive = TRUE, showWarnings = FALSE)
...
for (S_graph in unique(DF_Results_Graph_2graph$S)){
  for (alpha_graph in unique(DF_Results_Graph_2graph$alpha)){
    ...
    # PDF
    graph_path = file.path(paste0(getwd(), "/Gráficas/PDF/∆-Potencia MANTA-MANOVA (corr aleatoria)/", "alpha = ",
                                  sprintf("%1.3f", alpha_graph)))
    dir.create(graph_path, recursive = TRUE, showWarnings = FALSE)
    ...
    # PNG
    graph_path = file.path(paste0(getwd(), "/Gráficas/PNG/∆-Potencia MANTA-MANOVA (corr aleatoria)/", "alpha = ",
                                  sprintf("%1.3f", alpha_graph)))
    dir.create(graph_path, recursive = TRUE, showWarnings = FALSE)
...
# Directorio de la primera simulación a representar:
setwd(file.path(getwd(), "Resultados/Modelo mvnorm/Sim. 31-12-2023 20 h 10 min"))
...
# Directorio de la segunda simulación a representar:
setwd(file.path(getwd(), "Resultados/Modelo mvnorm/Sim. 08-01-2024 19 h 19 min"))
...
# Directorio de la unión de simulaciones:
...
DF_Results_Graph_2graph <- DF_Results_Graph_2graph[order(DF_Results_Graph_2graph$Método), ]
...
# => Filtramos las transformaciones no necesarias:
# .𝜏. unique(DF_Results_Graph_2graph$Datos)[c(1)] <=> Sin transformar
# .𝜏. unique(DF_Results_Graph_2graph$Datos)[c(1, 4, 5, 6)]] <=> Sin transformar + clr + log-ratio + sqrt
DF_Results_Graph_2graph <- DF_Results_Graph_2graph[DF_Results_Graph_2graph$Datos ==
                                                     c(unique(DF_Results_Graph_2graph$Datos)[1:4]), ]
# => Otros arreglos para la gráfica: etiquetas de Var + formato de Cor.
DF_Results_Graph_2graph$Var[DF_Results_Graph_2graph$Var == "equal"] <- "Equal"
...
# (a) + (b): Gráficas "∆-Potencia" comparativas entre MANTA-MANOVA (Tipos de datos a diferentes α).

## => Gráficas tipo Scatter Plot "∆-Potencia" entre MANTA y MANOVA agrupadas según la "Var" usada, y manteniendo el
##    "Tipo de datos - S" fijos, forzando una matriz de correlación inhomogénea (diferentes valores aleatorios
##    fuera de la diagonal):
Count_graph = 0
...
for (S_graph in unique(DF_Results_Graph_2graph$S)){
  for (Datos_graph in unique(DF_Results_Graph_2graph$Datos)){
      ...
      DF_temp <- DF_Results_Graph_2graph[DF_Results_Graph_2graph$S == S_graph
                                         & DF_Results_Graph_2graph$Datos == Datos_graph, ]
      
      # PDF
      graph_path = file.path(paste0(getwd(), "/∆-Potencia MANTA-MANOVA (corr aleatoria)/PDF"))
      dir.create(graph_path, recursive = TRUE, showWarnings = FALSE)
      
      pdf(file = file.path(graph_path,
                           paste0("Grid Var-alpha [S = ", toString(S_graph),
                                  " Datos = ", toString(Datos_graph), "].pdf")),
          width = 16, height = 12)
      
      p <- DeltaPotScatPlot_Method_mvnorm_5(DF_temp)
      print(p)
      
      dev.off()
      
      # PNG
      graph_path = file.path(paste0(getwd(), "/∆-Potencia MANTA-MANOVA (corr aleatoria)/PNG"))
      dir.create(graph_path, recursive = TRUE, showWarnings = FALSE)
      
      ppi = 600
      png(file = file.path(graph_path,
                           paste0("Grid Var-Datos [S = ", toString(S_graph),
                                  " Datos = ", toString(Datos_graph), "].png")),
          width = 16*ppi, height = 12*ppi, res = ppi)
...
for (S_graph in unique(DF_Results_Graph_2graph$S)){
  for (alpha_graph in unique(DF_Results_Graph_2graph$alpha)){
    ...
    # PDF
    graph_path = file.path(paste0(getwd(), "/Gráficas/PDF/Estudio Invarianza MANTA/", "alpha = ",
                                  sprintf("%1.3f", alpha_graph)))
    dir.create(graph_path, recursive = TRUE, showWarnings = FALSE)
    ...
    # PNG
    graph_path = file.path(paste0(getwd(), "/Gráficas/PNG/Estudio Invarianza MANTA/", "alpha = ",
                                  sprintf("%1.3f", alpha_graph)))
    dir.create(graph_path, recursive = TRUE, showWarnings = FALSE)
...
# Unión de los DF teniendo en cuenta solo MANTA:
DF_Results_Graph_2graph <- rbind(DF_Results_Graph_2graph1[DF_Results_Graph_2graph1$Método == "MANTA", ],
                                 DF_Results_Graph_2graph2[DF_Results_Graph_2graph2$Método == "MANTA", ])
...
## Se estudiarán los datos sin transformar y las transformaciones log-ratio, sqrt y clr:
DF_Results_Graph_2graph <- DF_Results_Graph_2graph[DF_Results_Graph_2graph$Datos ==
                                                     unique(DF_Results_Graph_2graph$Datos)[1:4], ]
